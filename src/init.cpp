 ...
 */
static const char* PALLADIUM_PID_FILENAME = "blockled.pid";
...
{
    const std::string URL_SOURCE_CODE = "<https://github.com/NotRin7/blockle>";
...
/** Sanity checks
 *  Ensure that Blockle is running in a usable environment with all
...
{
    // Make sure only a single Blockle process is using the data directory.
...
bool AppInitMain(NodeContext& node)
{
    ...
    if (fs::exists(config_file_path)) {
        ...
        if (configFile != NULL) {
            std::string strHeader = "# Blockle(PLM) config file:\n"
                                    "rpcuser=username\n"
                                    ...
                                    "\n"
                                    "# Addnodes:\n"
                                    "seednode=dnsseed.blockle-coin.store\n"
    ...
    if (gArgs.IsArgSet("-datadir") && !fs::path(gArgs.GetArg("-datadir", "")).is_absolute()) {
        LogPrintf("Warning: relative datadir option '%s' specified, which will be interpreted relative to the " /* Continued */
                  "current working directory '%s'. This is fragile, because if blockle is started in the future "
                  "from a different location, it will be unable to locate the current data files. There could "
                  "also be data loss if blockle is started while in a temporary directory.\n",
