...
const char * const PALLADIUM_CONF_FILENAME = "blockle.conf";
 ...
 * https://github.com/blockle/blockle/pull/16097#issuecomment-514627823
 */
...
bool ArgsManager::ParseParameters(int argc, const char* const argv[], std::string& error)
{
    ...
    for (int i = 1; i < argc; i++) {
        ...
        if (key == "-") break; //blockle-tx using stdin
        std::string val;
...
{
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\Blockle
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\Blockle
    // Mac: ~/Library/Application Support/Blockle
    // Unix: ~/.blockle
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Blockle";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/Blockle";
#else
    // Unix
    return pathRet / ".blockle";
#endif
#endif
}
...
std::string CopyrightHolders(const std::string& strPrefix)
{
    ...
    // Make sure Blockle Core copyright is not removed by accident
    if (copyright_devs.find("Blockle Core") == std::string::npos) {
        strCopyrightHolders += "\n" + strPrefix + "The Blockle Core developers";
    }
